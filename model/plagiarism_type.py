...
def identify_insert_replace(original_text, plagiarized_text, cosine_similarity):
    ...
    if cosine_similarity > 0.8:
        for tag, i1, i2, j1, j2 in s.get_opcodes():
            if tag == 'insert' and original_text in plagiarized_text:
                return True
            if tag in ['replace'] and percentage > 0.75:
                return True
    return False
...
def identify_text_change(original_text, plagiarized_text, cosine_similarity):
    ...
    if identify_insert_replace(original_text, plagiarized_text, cosine_similarity):
...
if __name__ == '__main__':
    ...
    plagiarized = 'The cat chased the mouse.'
    original = 'The mouse was chased by the cat.'
