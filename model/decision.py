...
class Decision:
    ...
    def generate_pdf_report(self,
                            file_name: str,
                            processed_list: list,
                            most_similar_documents: dict = {}):
        """
        Generate a PDF file with the given file_name,
        processed list and most_similar_documents.

        :param processed_list: the list generated by using the
        get_most_similar_document_sentences method from Processing class
        :param most_similar_documents: the dictionary generated by using the
        get_most_similar_documents method from Processing class
        :return: 0 if no error has ocurred, 1 if there was an error while
        ...
        title = ''
        plagiarism_pct = ''
        similar_documents_visible = 'block' if most_similar_documents else 'none'
        similar_documents = ''
        text = ''
        sentences = []
        ...
        if self.is_plagiarism(
            ...
            title = "PLAGIARISM NOT DETECTED\n\n"
            main_color = '#068422'

        # Plagiarism pct
        plagiarism_pct = f'{self.get_plagiarism_pct_sentences(processed_list):.0%}'

        # Most similar documents
        for document, similarity in most_similar_documents.items():
            if similarity > self.cosine_similarity_threshhold:
                similar_documents += f"{document} " \
                                          f"with similarity: " \
                                          f"{round(similarity, 2)*100}%<br>"
        ...
        for _, row in df.iterrows():
            if row['cosine_score'] < self.cosine_similarity_threshhold:
                text += row['sentence'] + ' '
                sentences.append(
                    f"<span style='text-decoration: underlined;'>Original sentence " \
                    f"(file {file_name}):</span><br>" \
                    f"'{row['sentence']}'<br><br>" \
                )
            else:
                text += f'<span style="background-color: #E18E8E;">{row["sentence"]}</span>'
                sentences.append(
                    f'<span style="text-decoration: underlined;">Plagiarized sentence '\
                    f"(file {file_name}):</span><br>" \
                    f"'{row['sentence']}'<br>" \
                    f'<span style="text-decoration: underlined;">Original sentence '\
                    f"(file {row['file_name']}):</span><br>" \
                    f"'{row['similar_sentence']}'<br><br>"
                )
                
        sentences = ''.join(sentences)


        file_template = file_template.format(file_name,
                                             main_color, title,
                                             main_color, plagiarism_pct,
                                             similar_documents_visible, similar_documents,
                                             text,
                                             sentences)
        
        
        pisa.showLogging()
        with open('./results/' + file_name + "_results.pdf", "wb") as pdf_file:
            pisa_status = pisa.CreatePDF(file_template, dest=pdf_file)
